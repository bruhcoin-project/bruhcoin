...
#if defined(NDEBUG)
# error "Bruhcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x3ca30dda559f7a1d521af09774f51d4be813a1ffdf26ac4366f2f556acbe3a4b");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Bruhcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Bruhcoin Signed Message:\n";
...
{
    // Bruhcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Bruhcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // Bruhcoin: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // Bruhcoin: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Bruhcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Bruhcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x3ca30dda559f7a1d521af09774f51d4be813a1ffdf26ac4366f2f556acbe3a4b");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Hurricane Irma Hit Florida 9/10/2017";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x5ec9595acc10f40884054ccaaaaba2760d19fe5f6397b8e7035013dbca9a6f19"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Bruhcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BruhcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BruhcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BruhcoinMiner(CWallet *pwallet)
{
    printf("BruhcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bruhcoin-miner");
    ...
    try { loop {
        ...
        printf("Running BruhcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BruhcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BruhcoinMiner, pwallet));
}
